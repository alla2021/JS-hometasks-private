...
//Привет! для начала, зачем тут confirm? здесь нужно использовать prompt и как сказано в условии задачи задавать вопрос 
//"Ваше число > N, < N или == N?". В зависимости от того, что указал пользователь, уменьшаете диапазон
//например пользователь говорит что число больше/меньше 50 (вводит знак > или <), 
//после этого делим диапазон пополам, и так дальше пока не выберет нужное
//все это лучше всего зациклить через цикл while
//и внутри цикла обрабатывать условие через if или switch

let n = 50;
let i = 0;
while (i < 6) {
    let num  = prompt('Ваше число >' + n +', <' + n + ' или = '+ n + '?');
    let a = 0;
    let b = 100;
    switch (num) {
        case '>':
...
// let y=50;
// for (i = 0; i < 6; i++) {
//     if (confirm('ваше число больше? ' + Math.floor(y) )) {
//         arr[0] = Math.floor(y);
//         }
//     else {
//         arr[1] = Math.floor(y);
